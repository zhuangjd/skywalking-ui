    ...
    for (const key of keys) {
      ...
      if (data.type === ServiceType.PREVIOUS) {
        ...
        const strKeyPre = `${preType.key === ObjectType.Database ? '' : str}
               ${preType.key === ObjectType.Service ? '' : preObject.label}-${key}`;
        obj[strKeyPre] = value;
      } else {
        ...
        const strKeyNext = `${nextType.key === ObjectType.Database ? '' : servicesLabel}
               ${nextType.key === ObjectType.Service ? '' : str}-${key}`;
